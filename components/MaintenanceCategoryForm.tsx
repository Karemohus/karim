        ...
        if (editingIssueId) {
            ...
            const updatedIssues = formData.commonIssues.map(issue =>
                issue.id === editingIssueId
                    ? { ...issue, name: trimmedIssueName, warrantyDays: newIssueWarranty }
                    : issue
            );
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                ...
                <form onSubmit={handleSaveIssue} className="grid grid-cols-1 sm:grid-cols-[1fr,120px,auto] gap-2 mb-3">
                    <input
                        type="text"
                        value={newIssueName}
                        onChange={(e) => setNewIssueName(e.target.value)}
                        placeholder="اسم العطل الشائع"
                    ...
                    />
                     <input
                        ...
                        title="أيام الضمان"
                        placeholder="أيام الضمان"
                    />
                    <div className="flex gap-2">
                         <button type="submit" className={`${editingIssueId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-green-600 hover:bg-green-700'} text-white font-bold p-2 rounded-lg transition-colors flex-shrink-0 flex items-center justify-center`} aria-label={editingIssueId ? "Update Issue" : "Add Issue"}>
                            {editingIssueId ? 'حفظ' : <PlusIcon className="w-6 h-6"/>}
                        </button>
                        {editingIssueId && (
                             <button type="button" onClick={handleCancelEdit} className="bg-gray-500 text-white font-bold p-2 rounded-lg hover:bg-gray-600 transition-colors flex-shrink-0 flex items-center justify-center" aria-label="Cancel Edit">
                                <XMarkIcon className="w-6 h-6"/>
                            </button>
                        )}
                    </div>
                </form>
                <div className="space-y-2 max-h-40 overflow-y-auto pr-1 border rounded-md p-2 bg-gray-50">
                    {formData.commonIssues.length > 0 ? (
                        formData.commonIssues.map((issue) => (
                            <div key={issue.id} className={`flex items-center justify-between p-2 rounded-md border transition-colors ${editingIssueId === issue.id ? 'bg-indigo-100 border-indigo-300' : 'bg-white'}`}>
                                <span className="text-gray-800">{issue.name}</span>
                                <div className="flex items-center gap-3">
                                  <span className="text-sm font-semibold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">{issue.warrantyDays} يوم ضمان</span>
                                  <button
                                      type="button"
